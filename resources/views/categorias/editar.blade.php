  ...
  <style>
    ...
    <div class="min-h-screen bg-gray-100 p-0 sm:p-12">
      <div class="mx-auto max-w-md px-6 py-12 bg-white border-0 shadow-lg sm:rounded-3xl">
        <h1 class="text-2xl font-bold mb-8">Actualizar los datos de la Categoria: {{$producto->id}}</h1>
        <form id="form" action="{{route('categorias.update', $producto)}}" method="POST">
          @csrf
          @method('put')
          ...
          <button type="button" class="w-full px-6 py-3 mt-3 text-lg text-white transition-all duration-150 ease-linear rounded-lg shadow outline-none bg-pink-500 hover:bg-pink-600 hover:shadow-lg focus:outline-none"><a href="{{route('categorias.principal')}}">Volver</a></button>
      </div>
    </div>
